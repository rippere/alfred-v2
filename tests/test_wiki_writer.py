...
def test_enrich_page_no_api_key_returns_false(ww, monkeypatch):
...
def test_enrich_page_unknown_entity_returns_false(ww, monkeypatch):
...
def test_enrich_page_extracts_new_facts_and_updates_vault(ww, cfg, monkeypatch):
    ...
    import alfred.core.anthropic_client as anthropic_client_mod
    monkeypatch.setattr(anthropic_client_mod, "get_client", lambda: fake_client)
...
def test_enrich_page_no_readable_sources_returns_false(ww, cfg, monkeypatch):
    ...
    called = []
    import alfred.core.anthropic_client as anthropic_client_mod
    monkeypatch.setattr(anthropic_client_mod, "get_client", lambda: called.append(True))
