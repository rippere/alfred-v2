"""Alfred MCP HTTP/SSE server — LAN-accessible transport for remote Claude Code clients.

This module exposes the same vault tools as server.py but over HTTP/SSE on 0.0.0.0:8765
so that laptop (or any LAN host) can connect without running its own Alfred daemon.
...
def run_server(config_path: Path, host: str = "0.0.0.0", port: int = 8765) -> None:
